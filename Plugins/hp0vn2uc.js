...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://lampaua.mooo.com/js/nws-client-es5.js?v07032026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'hp0vn2uc';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=hp0vn2uc');
    }
